...
export function UnreadSummaryDialog({
  ...
  const renderHistorySidebar = (isMobile = false) => {
    return (
      <div className={`flex min-h-0 flex-col ${isMobile ? 'h-full w-full' : ''} ${backgroundTheme === 'night' ? 'bg-slate-800/90' : 'bg-white/90'}`}>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={`max-w-5xl w-[96vw] max-h-[92vh] overflow-hidden border-slate-300/40 ${theme.contentBg} p-0`}>
        <div className="flex h-[92vh] max-h-[92vh] flex-col overflow-hidden">
          ...
          {/* Desktop Layout */}
          <div className="hidden lg:grid flex-1 min-h-0 grid-cols-[minmax(0,1.5fr)_360px] gap-0 overflow-hidden">
            <ScrollArea className={`min-h-0 ${theme.scrollBg} border-r`}>
            ...
            <Tabs defaultValue="summary" className="flex flex-1 flex-col overflow-hidden">
              <div className="border-b px-4 py-2 bg-white">
                <TabsList className="grid w-full grid-cols-3">
                  ...
                  <TabsTrigger value="select">Chọn tin</TabsTrigger>
                  <TabsTrigger value="history">Lịch sử</TabsTrigger>
                </TabsList>
              </div>

              <div className="flex-1 min-h-0 overflow-hidden">
                <TabsContent value="summary" className="h-full m-0 focus-visible:outline-none focus-visible:ring-0">
                ...
                <TabsContent value="select" className="h-full m-0 focus-visible:outline-none focus-visible:ring-0">
                  <ScrollArea className="h-full">
                    <div className="p-4">
                      {renderMessageSelector()}
                    </div>
                ...
                <TabsContent value="history" className="h-full m-0 focus-visible:outline-none focus-visible:ring-0">
